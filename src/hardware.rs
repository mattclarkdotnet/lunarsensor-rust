...
pub fn read_sensor(lockable_sensor: &LockableSensor) -> Result<f32> {
    // Using Anyhow and explicit matching instead of standard Error type and ?,
    // as my Rust isn't good enough to figure out the threading problems that
    ...
    return match locked_sensor.block_read(VEML6030_REG_ALS, &mut data) {
        Ok(_v) => Ok((((data[1] as u16) << 8) | data[0] as u16) as f32 * VEML6030_CONVERSION_FACTOR),
